...
class AitherDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some aitherd methods
    ...
    @classmethod
    def from_aither_conf(self, aither_dot_conf):
        from aither_config import AitherConfig
        config_text = AitherConfig.slurp_config_file(aither_dot_conf)
        creds = AitherConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = aitherlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = aitherlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = aitherlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_aitherd_proto_version_with_sentinel_ping)
