...
def is_valid_aither_address(address, network='mainnet'):
    ...
    aither_version = 112 if network == 'testnet' else 38
    ...
    try:
        ...
        # rescue from exception, not a valid Aither address
        return False

    if (address_version != aither_version):
        return False
...
# shims 'til we can fix the aitherd side
def SHIM_serialise_for_aitherd(sentinel_hex):
    ...
    # superblock => "trigger" in aitherd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # aitherd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    aitherd_hex = serialise(obj)
    return aitherd_hex


# shims 'til we can fix the aitherd side
def SHIM_deserialise_from_aitherd(aitherd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(aitherd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('aither.conf')
