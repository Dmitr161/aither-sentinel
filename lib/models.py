...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, aitherd):
        golist = aitherd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_aitherd(aitherd, item)
    ...
    @classmethod
    def import_gobject_from_aitherd(self, aitherd, rec):
        import decimal
        import aitherlib
        import inflection
        ...
        # shim/aitherd conversion
        object_hex = aitherlib.SHIM_deserialise_from_aitherd(object_hex)
        objects = aitherlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from aitherd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from aitherd, with every run
        ...
        # get/create, then sync payment amounts, etc. from aitherd - Aitherd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(aitherd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from aitherd! %s" % e)
            govobj.vote_delete(aitherd)
            return (govobj, None)
    ...
    def vote_delete(self, aitherd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(aitherd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, aitherd, signal, outcome):
        import aitherlib
        ...
        output = aitherd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = aitherlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(aitherd, signal)

    def sync_network_vote(self, aitherd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = aitherd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 aither addr, non-multisig
            if not aitherlib.is_valid_aither_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Aither address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (aitherlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/AitherDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import aitherlib
        obj_data = aitherlib.SHIM_serialise_for_aitherd(self.serialise())
    ...
    def prepare(self, aitherd):
        try:
            object_hash = aitherd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not aitherlib.is_valid_aither_address(addr, config.network):
    ...
    def hash(self):
        import aitherlib
        return aitherlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, aitherd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - aitherd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, aitherd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - aitherd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, aitherd):
        now = int(time.time())
        return (self.created_at < (now - aitherd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, aitherd):
        if self.is_expired(aitherd):
            return False

        return True

    def is_deletable(self, aitherd):
        if self.is_expired(aitherd):
            return True
