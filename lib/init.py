...
def has_aither_conf():
    ...
    valid_aither_conf = False

    # ensure aither_conf exists & readable
    #
    # if not, print a message stating that Aither Core must be installed and
    # configured, including JSONRPC access in aither.conf
    try:
        f = io.open(config.aither_conf)
        valid_aither_conf = True
    except IOError as e:
        print(e)

    return valid_aither_conf
...
def main():
    ...
    if not has_aither_conf():
        print("AitherCore must be installed and configured, including JSONRPC access in aither.conf")
        sys.exit(1)
